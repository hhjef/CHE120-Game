...
WINWIDTH = 640 # width of the program's     ow, in pixels
WINHEIGHT = 480 # height in pixels
HALF_WINWIDTH = int(WINWIDTH / 2)
HALF_WINHEIGHT = int(WINHEIGHT / 2)

GRASSCOLOR = (24, 255, 0)
NIGHTCOLOUR = (62, 67, 111) #JC the colour of night time
WHITE = (255, 255, 255)
RED = (255, 0, 0)
...
NIGHTTIME = 5        #JC how long the night lasts
DAYTIME = 10         #JC how long the day lasts
...
def runGame():
    ...
    daytimeStartTime = 0      #JC the screen is green at the beginning of the game
    nighttimeStartTime = 10   #JC the screen is blue after the beginning of the game
    gameOverMode = False      # if the player has lost
    gameOverStartTime = 0     # time the player lost
    winMode = False           # if the player has won
    # defining daytime and night time
    daytime = DISPLAYSURF.fill(GRASSCOLOR)
    nighttime = DISPLAYSURF.fill(NIGHTTIME)
    backgrounds = [daytime, nighttime]
    elapsed_time = pygame.time.get_ticks()
    index = (elapsed_time // BACKGROUND_DURATION) % len(backgrounds)
    pygame.display.flip()
    ...
    while True: # main game loop
        ...
        DISPLAYSURF.fill(colors[color_index])
...
def nighttime(FPSCLOCK):
    if FPSCLOCK == 1:
        DISPLAYSURF.fill(NIGHTCOLOUR)
    else:
        DISPLAYSURF.fill(GRASSCOLOR)
